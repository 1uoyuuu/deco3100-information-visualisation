...
Plotly.d3.csv(
  ...
  (calorieData) => {
    ...
    const meatCalorie = unpack(
      calorieData,
      "Daily Calorie from Animal Product"
    );
    var traceOne = {
      x: calorieYear,
      y: totalCalorie,
      ...
      hovertemplate:
        "Energy supply from<br>all foods: <b>%{y:,} kcal</b><extra></extra>",
    };
    var traceTwo = {
      x: calorieYear,
      y: meatCalorie,
      ...
      stackgroup: "one",
      hovertemplate:
        "Energy supply from<br>animal products: <b>%{y:,} kcal</b><extra></extra>",
    };
...
Plotly.d3.csv("src/data-sets/habitat-change-by-2050.csv", (habitatData) => {
  ...
  const values = unpack(habitatData, "Habitat Change")
...
var data = [
  {
    ...
    hole: 0.5, // make sure it's a donut shape
    name: "67% chance",
    hovertemplate:
      "Sector: <b>%{label}</b><br>" +
  ...
  var data = [
    {
      ...
      hovertemplate:
        "<b>%{x} m²</b> land is required to produce<br>1000 kilocalories of <b>%{y}</b>.<extra></extra>",
    },
  ];
  var layout = {
    height: 500,
...
Plotly.d3.csv(
  ...
  (landData) => {
    ...
    var traceOne = {
      ...
      hovertemplate:
        "<b>%{x} billion hectares Cropland</b> is required if our diet pattern is<br><b>%{y}</b>.<extra></extra>",
    };
    var traceTwo = {
      y: [dietPattern[0]],
      x: [pasture[0]],
      ...
      hovertemplate:
        "<b>%{x} billion hectares Pasture land</b> is required if our diet pattern is<br><b>%{y}</b>.<extra></extra>",
    };
    ...
    Plotly.newPlot(
      "land-required-for-different-diets-chart",
      data,
      layout,
      config
    ).then(function () {
  ...
  (emissionData) => {
    ...
    var data = [
      {
        ...
        hovertemplate:
          "Mode: <b>%{x}</b><br>Emission: <b>%{y} billion tonnes</b><extra></extra>",
      },
    ];
    ...
    for (var i = 0; i < method.length; i++) {
      var percentageChange =
        ((emission[0] - emission[i]) / emission[0]).toFixed(2) * 100;
      var annotation = {
        x: method[i],
        y: emission[i],
